#!/bin/bash
# Updated Intelligent Router - Grok as default, all models can collaborate

sudo tee /home/ubuntu/signaturebrain-backend/controllers/brainController.js > /dev/null <<'ENDOFFILE'
...
function analyzeTask(message) {
  ...
  // Complex questions → All models collaborate
  if (msg.match(/analyze|compare|evaluate|explain why|pros and cons|what do you think|opinion/)) {
    return {
      type: "analysis",
      strategy: "collaborate",
      models: [
        { api: "grok", model: "grok-beta", role: "primary" },
        { api: "anthropic", model: "claude-3-5-sonnet-20241022", role: "analyzer" },
        { api: "openai", model: "gpt-4-turbo-preview", role: "critic" }
      ]
    };
  }
  ...
  // Research → Multiple models for accuracy
  if (msg.match(/what is|who is|when did|research|fact|history/)) {
    return {
      type: "research",
      strategy: "collaborate",
      models: [
        { api: "grok", model: "grok-beta", role: "researcher" },
        { api: "anthropic", model: "claude-3-5-sonnet-20241022", role: "verifier" }
      ]
    };
  }

  // Default → Grok (base model)
  return {
    type: "general",
    strategy: "single",
    model: { api: "grok", model: "grok-beta" }
  };
}
...
async function routeToModel(message, taskAnalysis) {
  ...
  if (api === "anthropic") {
    ...
    return await callGrok(message, model);
  }
  return { reply: "Model not available" };
}

async function collaborativeResponse(message, taskAnalysis) {
  console.log(`🤝 Collaboration: ${taskAnalysis.models.length} models working together`);
  ...
  for (const modelConfig of taskAnalysis.models) {
    try {
      ...
      } else if (modelConfig.api === "grok") {
        response = await callGrok(message, modelConfig.model);
      }
      ...
      console.error(`⚠️ ${modelConfig.api} (${modelConfig.role}) failed:`, err.message);
    }
  }

  if (responses.length === 0) {
    return { reply: "No responses received from any model" };
  }
  ...
  console.log(`🔄 Synthesizing ${responses.length} model responses...`);

  let synthesis = `Models used: ${responses.map(r => r.model).join(', ')}\n\n`;
...
async function callGrok(message, model) {
  ...
  if (!response.ok) {
    throw new Error(`Grok: ${data.error?.message || response.status}`);
  }

  const text = data?.choices?.[0]?.message?.content;
  if (!text) {
    throw new Error("No text in Grok response");
  }
...
echo "✅ Updated! Grok is now the default model"
echo ""
echo "Routing:"
echo "  - General messages → Grok (default)"
echo "  - Code → GPT-4"
echo "  - Creative → Claude"
echo "  - Analysis → All 3 collaborate"
echo "  - Research → Grok + Claude verify"
echo ""
echo "Add to .env:"
...
echo ""
echo "Restart: pm2 restart all && pm2 flush"
